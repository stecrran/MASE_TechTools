...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        // Sample code, to test our ProductSuggestionCrudRepository.
        ProductSuggestionCrudRepository repo = context.getBean(ProductSuggestionCrudRepository.class);

        System.out.println("\nAll ProductSuggestion objects:");
        for (ProductSuggestion ps: repo.findAll()) {
            System.out.println(ps);
        }

        ProductSuggestion insertedPs = repo.save(new ProductSuggestion("Skis", 600, 1000));
        long id = insertedPs.getId();
        repo.modifyPrice(id, 600.99);
        repo.modifySales(id, 1001);
        ProductSuggestion modifiedPs = repo.findById(id).get();
